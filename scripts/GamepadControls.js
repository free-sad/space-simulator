...
THREE.GamepadControls = function ( object ) {
	...
	this.pollGamepads = function() {
      ...
      for(var i = 0; i < rawGamepads.length; i++) {
        if(rawGamepads[i].mapping) { //prevent keyboards showing as game controllers
          g = rawGamepads[i];
          break;
        }
      }
      ...
      if(g) {
  			this.lon += this.filter( g.axes[ 2 ] ); //Right joystick (vertical)
  			this.lat -= this.filter( g.axes[ 3 ] ); //Right joystick (horizontal)
  			...
  			this.dir.set(
  				this.filter( g.axes[ 0 ] ), //Left joystick (vertical)
  				this.filter( g.buttons[ 4 ].value ) - this.filter( g.buttons[ 5 ].value ), //LB - RB
  				this.filter( g.axes[ 1 ] ) //Left joystick (horizontal)
  			);
